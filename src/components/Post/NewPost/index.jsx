...
const NewPost = ({ setShowModal, hideCard = false }) => {
  ...
  const [createPostTypedData, { loading: typedDataLoading }] = useMutation(
    ...
    {
      async onCompleted({ createPostTypedData }) {
  ...
  const createPost = async () => {
    ...
    const { IpfsHash: path } = await uploadToIPFS({
      version: '1.0.0',
      metadata_id: uuid(),
      description: trimify(postContent),
      content: trimify(postContent),
      external_url: `https://narrativedex.xyz/u/${currentUser?.handle}`,
